...
!!! quote "Consequence for contributors"
...
`acme_` are equivalent. See [Prefix strategy](../architecture/prefix-strategy.md).
...
row, and an outbox event. See [Subscriptions](subscriptions.md).
...
The first hit wins (expired overrides are skipped). See [Entitlements](entitlements.md).

## Next

- [Catalog & plans](catalog.md)
